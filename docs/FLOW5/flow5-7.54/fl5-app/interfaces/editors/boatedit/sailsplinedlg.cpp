...
void SailSplineDlg::fillSectionModel()
{
    ...
    {
        Vector3d pos = pNS->sectionPosition(is);
        ind = m_pSectionModel->index(is, 0, QModelIndex());
        m_pSectionModel->setData(ind, pos.x * Units::mtoUnit());

        ind = m_pSectionModel->index(is, 1, QModelIndex());
        m_pSectionModel->setData(ind, pos.y * Units::mtoUnit());

        ind = m_pSectionModel->index(is, 2, QModelIndex());
        m_pSectionModel->setData(ind, pos.z * Units::mtoUnit());
...
void SailSplineDlg::readSectionData()
{
    ...
    for(int is=0; is<m_pSectionModel->rowCount(); is++)
    {
        ...
        if(pItem) {
            strong = pItem->text().replace(" ","");
            d = strong.toDouble(&bOK);
            if(bOK) pos.x = d / Units::mtoUnit();
        }
        ...
        if(pItem) {
            strong = pItem->text().replace(" ","");
            d = strong.toDouble(&bOK);
            if(bOK) pos.z = d / Units::mtoUnit();
        }
        
        pNS->setSectionPosition(is, pos);
        ...
        if(pItem) {
            strong = pItem->text().replace(" ","");
            d = strong.toDouble(&bOK);
            if(bOK) pNS->setSectionAngle(is, d);
        }
    }
}
...
void SailSplineDlg::makeTables()
{
    ...
    m_pSectionModel->setColumnCount(5);
    m_pSectionModel->setHeaderData(0, Qt::Horizontal, "x ("+Units::lengthUnitQLabel()+")");
    m_pSectionModel->setHeaderData(1, Qt::Horizontal, "y ("+Units::lengthUnitQLabel()+")");
    m_pSectionModel->setHeaderData(2, Qt::Horizontal, "z ("+Units::lengthUnitQLabel()+")");
    m_pSectionModel->setHeaderData(3, Qt::Horizontal, "angle ("+ DEGch +")");
    m_pSectionModel->setHeaderData(4, Qt::Horizontal, "Actions");
    m_pSectionModel->setActionColumn(4);
    ...
    m_pSectionDelegate->setDigits({3,3,3,1,0});
    m_pSectionDelegate->setItemTypes({XflDelegate::DOUBLE, XflDelegate::DOUBLE, XflDelegate::DOUBLE, XflDelegate::DOUBLE, XflDelegate::STRING});
    m_pSectionDelegate->setActionColumn(4);
