...
Vector3d SailSpline::sectionPoint3d(int isec, double xrel) const
{
    if(isec<0 || isec>=sectionCount()) return Vector3d();
    Spline const *spl = m_Spline.at(isec);
    Vector2d p = spl->splinePoint(xrel);
    Vector3d pt(p.x, p.y, m_Position.at(isec).z); // Z is still absolute/base
    // The key change: add X and Y offsets before/during rotation
    // If we assume sectionPosition is the LE position:
    pt.set(p.x + m_Position.at(isec).x, p.y + m_Position.at(isec).y, m_Position.at(isec).z);
    pt.rotateY(m_Position.at(isec), m_Ry.at(isec));
    return pt;
}


Vector3d SailSpline::sectionPointAtParam(double xrel, double zrel) const
{
    ...
    Vector3d pt0 = sectionPoint3d(isec, xrel);
    Vector3d pt1 = sectionPoint3d(isec+1, xrel);
...
{
    return sectionPointAtParam(xrel, zrel);
}
...
{
    if(m_Position.size()<2) return Vector3d(0,0,1);

    Vector3d theTack = sectionPoint3d(0, 0.0);
    Vector3d theHead = sectionPoint3d(sectionCount()-1, 0.0);
...
bool SailSpline::makeBSpline3d(int ispl, BSpline3d &spline3d) const
{
    ...
    for(int ic=0; ic<spl->ctrlPointCount(); ic++)
    {
        ...
        pt3d.set(pt.x + m_Position.at(ispl).x, pt.y + m_Position.at(ispl).y, m_Position.at(ispl).z);
